      ...
      <StyledTable>
        ...
        <tbody>
          {sortedFiles.map(file => (
            // Clicking anywhere on the row (including icon/download) calls handleRowClick.
            <BodyRow key={file.id} onClick={() => handleRowClick(file)}>
              <BodyCell>
                {/* placeholder text for icon (e.g. file type icon later) */}
                <span style={{ marginRight: "0.5rem" }}>:D</span>
                {file.name}
              </BodyCell>
              <BodyCell>{file.dateAdded}</BodyCell>
              <BodyCell>{file.dateModified}</BodyCell>
              <BodyCell>{formatSize(file.sizeBytes)}</BodyCell>
              {/* placeholder text for download icon/button (currently just text) */}
              <BodyCell>Download</BodyCell>
            </BodyRow>
          ))}
        </tbody>
      </StyledTable>
